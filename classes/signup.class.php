...
class SignUp extends Generic {
	...
	private function validate() {
		...
		if(empty($this->settings['colemanid'])) {
			$this->error .= '<li>'._('You must enter your Coleman ID#.').'</li>';
		}

		if(empty($this->settings['name'])) {
			$this->error .= '<li>'._('You must enter your name.').'</li>';
		}
		if(empty($this->settings['role'])) {
			$this->error .= '<li>'._('You must enter your role.').'</li>';
		}
		if(empty($this->settings['department'])) {
			$this->error .= '<li>'._('You must enter your department.').'</li>';
		}
		
		if(empty($this->settings['phonenum'])) {
			$this->error .= '<li>'._('You must enter your cell number. (xxx-xxx-xxxx)').'</li>';
		}					
		...
		if(strlen($this->settings['password']) < 8) {
			$this->error .= '<li>'._('Your password must be at least 8 characters.').'</li>';
		}
		
		if(strlen($this->settings['phonenum']) == 12) {
			$this->error .= '<li>'._('Your cell number must be entered like this xxx-xxx-xxxx').'</li>';
		}
	...
	private function register() {
		...
		if(empty($this->error)) {
			...
			$sql = "INSERT INTO login_users (user_level, name, colemanid, role, department, email, phonenum, username, password)
						VALUES (:user_level, :name, :colemanid, :role, :department, :email, :phonenum, :{$this->username_type}, :password);";
			$params = array(
				':user_level' => parent::getOption('default-level'),
				':name'       => $this->settings['name'],
				':colemanid'  => $this->settings['colemanid'],
				':role'       => $this->settings['role'],
				':department' => $this->settings['department'],
				':email'      => $this->settings['email'],
				':phonenum'   => $this->settings['phonenum'],
